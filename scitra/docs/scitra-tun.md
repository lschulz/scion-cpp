...
On a working SCION host, Scitra-TUN only requires a few options on the command line. All command
line options can alternatively be read from a file in TOML or INI syntax by giving the `--config`
option. If an option is specified in the config file and on the command line, the command line takes
precedence.
